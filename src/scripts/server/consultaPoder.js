document.getElementById('buscaPoder').addEventListener('submit', async function(event) {
    ...
    try {
        const results = await fetch(`http://localhost:3000/consultaPoder?${queryString}`, {
            method: 'GET',
            headers: {
        ...
        if (results.ok) {
            const poder = await results.json();
            console.log(poder);
        } else {
