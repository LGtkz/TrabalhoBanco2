document.getElementById('deletaMusica').addEventListener('submit', async function(event) {
    ...
    try {
        const results = await fetch(`http://localhost:3000/deletaMusica?${queryString}`, {
            method: 'DELETE',
            headers: {
        ...
        if (results.ok) {
            ...
            errorMsg.innerHTML = 'Erro ao deletar musica!';
        }
    } catch (error) {
