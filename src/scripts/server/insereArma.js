document.getElementById('insereArma').addEventListener('submit', async function(event) {
    ...
    try {
        const response = await fetch('http://localhost:3000/insereArma', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            window.location.href = 'Form_armaCriar.html';
        } else {
