document.getElementById('deletaArma').addEventListener('submit', async function(event) {
    ...
    try {
        const results = await fetch(`http://localhost:3000/deletaArma?${queryString}`, {
            method: 'DELETE',
            headers: {
        ...
        if (results.ok) {
            ...
            errorMsg.innerHTML = 'Erro ao deletar arma!';
        }
    } catch (error) {
