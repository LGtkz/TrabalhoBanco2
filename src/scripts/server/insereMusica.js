document.getElementById('insereMusica').addEventListener('submit', async function(event) {
    ...
    try {
        const response = await fetch('http://localhost:3000/insereMusica', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            window.location.href = 'Form_musicaCriar.html';
        } else {
