document.getElementById('buscaCacador').addEventListener('submit', async function(event) {
    ...
    try {
        const results = await fetch(`http://localhost:3000/consultaCacador?${queryString}`, {
            method: 'GET',
            headers: {
        ...
        if (results.ok) {
            const cacador = await results.json();
            console.log(cacador);
        } else {
