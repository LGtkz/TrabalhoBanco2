document.getElementById('insereLugar').addEventListener('submit', async function(event) {
    event.preventDefault(); 
    const formData = new FormData(this);
    const data = Object.fromEntries(formData.entries());

    try {
        const response = await fetch('http://localhost:3000/insereLugar', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            window.location.href = 'Form_lugarCriar.html';
        } else {
