document.getElementById('insereDemonio').addEventListener('submit', async function(event) {
    ...
    try {
        const response = await fetch('http://localhost:3000/insereDemonio', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            window.location.href = 'Form_demonioCriar.html';
        } else {
