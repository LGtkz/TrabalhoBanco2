document.getElementById('insereCacador').addEventListener('submit', async function(event) {
    ...
    try {
        const response = await fetch('http://localhost:3000/insereCacador', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            window.location.href = 'Form_cacadoresCriar.html';
        } else {
