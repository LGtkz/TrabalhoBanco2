document.getElementById('deletaDemonio').addEventListener('submit', async function(event) {
    ...
    try {
        const results = await fetch(`http://localhost:3000/deletaDemonio?${queryString}`, {
            method: 'DELETE',
            headers: {
        ...
        if (results.ok) {
            ...
            errorMsg.innerHTML = 'Erro ao deletar Demonio!';
        }
    } catch (error) {
