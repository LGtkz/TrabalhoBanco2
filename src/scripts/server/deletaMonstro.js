document.getElementById('deletaMonstro').addEventListener('submit', async function(event) {
    ...
    try {
        const results = await fetch(`http://localhost:3000/deletaMonstro?${queryString}`, {
            method: 'DELETE',
            headers: {
        ...
        if (results.ok) {
            ...
            errorMsg.innerHTML = 'Erro ao deletar lugar!';
        }
    } catch (error) {
