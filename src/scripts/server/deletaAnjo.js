document.getElementById('deletaAnjo').addEventListener('submit', async function(event) {
    ...
    try {
        const results = await fetch(`http://localhost:3000/deletaAnjo?${queryString}`, {
            method: 'DELETE',
            headers: {
        ...
        if (results.ok) {
            ...
            errorMsg.innerHTML = 'Erro ao deletar anjo!';
        }
    } catch (error) {
