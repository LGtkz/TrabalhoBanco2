document.getElementById('insereMonstro').addEventListener('submit', async function(event) {
    ...
    try {
        const response = await fetch('http://localhost:3000/insereMonstro', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            window.location.href = 'Form_feitiçoCriar.html';
        } else {
